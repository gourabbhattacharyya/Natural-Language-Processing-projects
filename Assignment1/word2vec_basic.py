...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  window_size = (skip_window * 2) + 1 #determine the size of the window that needs to be skipped
  ...
  for i in range(batch_size // num_skips):
      contextWord = skip_window  #target lebel at buffer center
      contextWords = [skip_window]

      for j in range(num_skips):
          while contextWord in contextWords:
              contextWord = random.randint(0, window_size - 1)

          contextWords.append(contextWord)
          batch[i * num_skips + j] = buffer[skip_window]
          labels[i * num_skips + j, 0] = buffer[contextWord]
...
def build_model(sess, graph, loss_model):
  ...
  with graph.as_default():
    ...
    if loss_model == 'cross_entropy':
      print("embed value", embed)
      print("true_w value", true_w)
...
if __name__ == '__main__':
  ...
  print("unigram_cnt", unigram_cnt[:5])
  print("total", total)
  print("unigram_prob", unigram_prob[:5])
  ...
  print("printing done after generate batch")
  ####################################################################################
