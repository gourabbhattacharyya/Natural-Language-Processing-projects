...
class ParsingSystem:
    ...
    def apply(self, c, t):
        ...
        =================================================================
        """
        w1 = c.getStack(1)      #retrieve the 2nd element from stack
        w2 = c.getStack(0)      #retrieve the 1st element from stack

        if t.startswith("L"):
            c.addArc(w2, w1, t[2:-1])       #add arc
            c.removeSecondTopStack()        #remove from stack
        elif t.startswith("R"):
            c.addArc(w1, w2, t[2:-1])
            c.removeTopStack()
        else:
            c.shift()               #remove the top element from the buffer and add it to the stack
